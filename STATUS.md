...
- **2026-05-19 #3** — **v8 path-0 hardening reviewer: APPROVED**. Fresh adversarial reviewer (Task #65) independently verified all 10 v8 checks pass: Part A gate (`order_type == "market"`) is present at both conditions; Part B1 close-direction sign correct (BUY→SELL @ bid; SELL→BUY @ ask); Part B2 stale_set engages on retcode=10018 and is consulted in BOTH path-0 branches (the load-bearing v7-anomaly-mechanism guard); marker test extension validated end-to-end; schema 1.2→1.3 back-compat clean (v1.0 fixture is strict superset of v1.1 absent-field set, implicitly covers v1.1); (symbol, volume, side) root-cause claim sound (LOT_SIZE locked at 0.01 confirmed at scripts/record_fills.py:380; v7 commit `3e72fe3` matcher uses volume+side, claimed_deal_tickets session-scoped so zero protection against prior-session residuals); confidence flag "high" justification lives in module docstring at lines 269-283 citing the 2-capture replication; AST regression + cost-reconciliation + cross-process determinism guards all pass. Full suite **786 + 1 skipped + 2 deselected** bit-exact match to impl agent's claim. **Single LOW finding**: latency floor 50ms in the marker test leaves a [12.6ms, 50ms] gap — v7 anomaly was at 12.6ms, typical FTMO RTT ~150-200ms, so 75-100ms floor would be a tighter discriminator. Non-blocking; reviewer recommends tightening only after several clean captures empirically confirm typical RTT bottom. Added to deferred ledger as a follow-up tied to the empirical-confirmation trigger. **v8 functional acceptance gate**: user runs `PROPFARM_LIVE_TEST=1 pytest tests/scripts/test_live_broker_validation.py` on the VPS — clear pass/fail signals + structured stderr surfaces for diagnosis if anything fails. Commit: this entry.
...
| Wave 6d `test_event_calibration_does_not_mutate_global_registry` snapshots scalar fields only (`baseline_bps`, `news_multiplier`, `stress_multiplier`) — does NOT check `id()` or full `model_dump()`. A future change that swaps the entry's identity while keeping those 3 values identical (no-op refactor) would silently pass even if the swap leaked. Wave 6d reviewer LOW; non-blocking per reviewer ("the current test catches the specific regression class that motivated it") | tests/sim/test_stress_replay | Strengthen to `assert SPREAD_CALIBRATIONS[sym].model_dump() == pre_dump` + `assert id(SPREAD_CALIBRATIONS[sym]) == pre_id` across all FX-major entries. Combine with the pre-commit mypy fix below — `test_stress_replay.py` currently has 7 pre-existing mypy import-not-found errors that block any edits to the file |
| Pre-commit mypy hook isolated-venv missing project install. The mirrors-mypy hook spawns an isolated venv with only the `additional_dependencies` listed in `.pre-commit-config.yaml`; the `propfarm` package itself is NOT installed, so `from propfarm.sim.* import ...` returns `Cannot find implementation or library stub`. Confirmed pre-existing across `tests/sim/test_spread.py` (5 errors), `tests/sim/test_stress_replay.py` (7 errors) — Wave 6d commit `6dd9bba` somehow shipped past this; later edits to either file are blocked at commit-time. Discovered 2026-05-19 while applying Wave 6d reviewer LOW #3 | tooling | Fix the mypy hook to install the project itself. Two options: (a) add `propfarm @ file:///path/to/repo` to `additional_dependencies` (path-fragile); (b) switch the mypy hook from `repo: mirrors-mypy` to `repo: local` with `language: system` so it uses the project venv (cleanest). Until fixed, any commit that touches `tests/sim/test_*.py` will fail pre-commit and must either bypass via `--no-verify` (against safety protocol) OR be split so the .py edits go through a separate path |
| v8 marker-test latency floor 50ms leaves a [12.6ms, 50ms] gap relative to v7 anomaly signature (12.6ms) and typical FTMO RTT (~150-200ms). The 50ms floor still catches the observed v7 signature but a 75-100ms floor would be a tighter discriminator. v8 reviewer-flagged LOW non-blocking 2026-05-19 #3 | scripts/test_live_broker_validation | Optional follow-up — after several clean captures empirically confirm typical RTT bottom is well above 75-100ms, tighten `SUSPICIOUS_LATENCY_FLOOR_MS` in `tests/scripts/test_live_broker_validation.py` from 50.0 to 75-100.0. Don't tighten preemptively — risks false-positives on slow-network days |

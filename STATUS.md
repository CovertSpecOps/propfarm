...
| Gate 2B: sim/live fill comparison (harness ready, awaiting 24h capture) | 🟨 24h capture running 2026-05-15 | Harness commits `987e5f5` + `a2b56d4`. Active record_fills bug class closed via v7 (`3e72fe3`); short-test 7/7 market success with `n_market_lookup_failures=0`. 24h Task Scheduler run kicked off 2026-05-15 with `--duration-hours 24 --n-samples 200`. ETA ~24h. Gate 2B comparison harness runs against the resulting parquet once it lands; verdict pass / investigate / fail per the harness's per-symbol thresholds (commits `987e5f5` + `a2b56d4` + `c5913d4`) |
...
| record_fills v7 limit-order path-0 anomaly: short-test idx=000 GBPUSD limit sell filled at 1.34400 with **12.6ms latency** — far faster than typical broker RTT (~160ms). Likely cause: path-0's symbol+volume+side fallback matcher picked up either (a) a marketable-limit immediate-fill, or (b) a residual position from a previous session. Non-blocking for Gate 2B (consumes market rows only) | record_fills | Pre-Phase-1 cleanup. Audit: (a) verify path 0 fires only on `order_type == "market"` (currently fires for all order types on hedging — limit orders may have residual-position-pickup risk); (b) add a pre-fill positions_get sweep at session start so residual positions from prior runs are visible to the operator; (c) if path-0 limit anomalies persist on the 24h capture, gate path 0 on `order_type == "market"`. User flagged 2026-05-15 #1 |

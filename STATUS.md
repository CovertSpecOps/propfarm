...
| Gate 2B: sim/live fill comparison | 🟨 INVESTIGATE 2026-05-18 round 1 | Capture `bbf710b3...` clean. Round 1 calibration (commits `356e096` + `f87a748`) moved FAIL → INVESTIGATE. Sole remaining reason: `spread_p95_investigate p95=1.4275 pip threshold=1.0 pip fail_band=1.5 pip`, driven by 3-4 pre-Sunday-reopen widening outliers (21:18-21:55 UTC) — real model gap (no pre-rollover widening term in sim's spread model), NOT a calibration miss. Pre-cal FAIL reasons (fill_price p95 EURUSD/GBPUSD, spread bias, latency bias) all gone. Per-side bias collapsed for 3/4 panes; GBPUSD-sell residual still positive due to v7 session-start outlier (tracked in deferred ledger). Reviewer pass running on round-1 outputs. Next move pending reviewer verdict + user direction: either (a) accept INVESTIGATE as Phase 0 PASS condition (Wave 6d unblocks), OR (b) add `pre_rollover_multiplier` to spread model + round-2 re-run against the same parquet |
...
| Gate 2B round-1 calibration: `vol_coef=0` removes vol-dependence from EURUSD/GBPUSD slippage. Defensible from thin data (n=119 markets across one 24h capture; slope-vs-noise can't be resolved). Stress-replay scenarios on FX majors now derive slip purely from size_term + stress_multiplier×base (=0) + NY-close minute_term. Reviewer-flagged 2026-05-18 round-1 LOW | sim/slippage | Round 2 — re-introduce a small positive `vol_coef` when longer-window data resolves the slope question. Suggested gate: 2nd capture week-spanning + residual std within ±0.1 pip on a 500+ row sample. Module-level comment in test_slippage.py points back to this calibration round |
| Gate 2B round-1 calibration: spread INVESTIGATE p95=1.43 pip driven by ~4 NY-close / pre-rollover widening outliers (21:18-21:55 UTC weekday). The live broker widens the spread before the NY session closes; sim has no pre-rollover widening term. Same phenomenon also affects Sun ~22:00 UTC pre-reopen on weekend-spanning captures (not in this capture) | sim/spread | Round 2 — add a session-aware `pre_rollover_multiplier` (mirror the existing session-open machinery but apply at ~21:00 UTC). Target: spread p95 ≤ 1.0 pip → PASS verdict on a re-run against the same 2026-05-18 parquet |

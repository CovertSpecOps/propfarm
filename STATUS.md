...
| ~~Wave 6d `test_event_calibration_does_not_mutate_global_registry` snapshots scalar fields only~~ — **RESOLVED 2026-05-19 fix-up**: strengthened to `model_dump()` equality + `id()` preservation across EURUSD AND GBPUSD entries for BOTH registries. Catches a future no-op refactor that swaps entry identity while keeping scalar field values | tests/sim/test_stress_replay | Closed |
| `propfarm.data.ingest._hour_ts_from_path` silently skips paths with `len(rel.parts) != 4` (returns `None` and the ingest loop drops the file with a warning). Same strict-4-parts check in `scripts/bulk_fetch_dukascopy.py:_scan_existing`. If future work adds sidecar metadata (e.g. `_meta.json` per day, or a checksum file alongside the `.bi5`), both the ingest reader AND the bulk-fetcher resume scan will silently ignore the sidecar. Internal consistency holds today; flagged by Wave 6d-style reviewer NIT 2026-05-19 | data-layer | Forward-defensive: if Phase 1+ work needs sidecar metadata at the cache layer, tighten BOTH `_hour_ts_from_path` AND `_scan_existing` to either (a) accept the sidecar via a separate path-shape rule, OR (b) raise on unexpected path shapes instead of silently skipping. Today's silent-skip is the right Phase-0 behaviour |
| ~~Pre-commit mypy hook isolated-venv missing project install~~ — **RESOLVED 2026-05-19 fix-up**: switched the mypy hook from `repo: mirrors-mypy` (isolated venv) to `repo: local` with `language: system` (uses the project venv that has `propfarm` installed via `pip install -e ".[dev]"`). Added `src/propfarm/py.typed` PEP-561 marker so mypy treats the package as type-checked. Full pre-commit + mypy now passes on every .py file in the repo | tooling | Closed |

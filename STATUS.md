...
| Gate 2B: sim/live fill comparison (harness ready, awaiting next 24h capture) | 🟨 capture restart pending Monday 2026-05-18 | Harness commits `987e5f5` + `a2b56d4` + `c5913d4`. Active record_fills bug class closed via v7 (`3e72fe3`). First 24h attempt (run `56c954a`, Fri 2026-05-15 14:00 UTC) interrupted by Friday FX close at 21:00 UTC — `n_filled_market=28 < 100` threshold. See 2026-05-15 #2 session entry. Runbook §0.5 (NEW) documents the safe-start window (Sun 22:00 UTC → Thu 21:00 UTC for a 24h capture). Operator restarts Sunday 22:00 UTC or later (FX Sunday open) for full weekday coverage |

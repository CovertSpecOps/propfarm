...
| record_fills path-2 `DEAL_ENTRY_IN` filter may falsely drop valid deals on non-FTMO netting brokers (currently moot for FTMO hedging since path 0 fires first; latent risk for FundedNext / FundingPips netting accounts) | record_fills | Before any Phase B prop-firm challenge, run live_broker_validation marker against the second firm's MT5 demo. If `[record_fills:lookup_probe_h] returned > 0` but `[record_fills:lookup_probe_h_in] returned=0`, the filter is rejecting a valid entry deal. Candidate fix: broaden the comparison (handle missing `entry` field, log unexpected values), and add a path-2 raw-vs-filtered diagnostic probe in production (not just the v5-diagnostic). User flagged 2026-05-14 #5 |

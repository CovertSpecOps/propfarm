...
| Gate 2B: sim/live fill comparison | 🟥 FAIL 2026-05-18 — cost models need recalibration | Capture `bbf710b3...` clean (199 fills / 119 market / n_market_lookup_failures=0). Harness verdict FAIL on 4 reasons (per-symbol fill_price p95 exceeded EURUSD 1.29 / GBPUSD 2.11 vs 0.50 threshold; spread + latency systematic-bias t-tests). Per-side smoking gun: BUY mean=−0.96 pip / SELL mean=+0.98 pip → sim overstates adverse-fill cost by ≈1 pip per side on FX majors. See 2026-05-18 #1 session entry for the full residual breakdown. Reviewer dispatched. After reviewer + user direction, recalibrate `propfarm.sim.slippage` + `propfarm.sim.spread` + fill engine's default `execution_latency_ms`, then re-run against the SAME parquet (no new capture required unless calibration reveals a data gap) |
